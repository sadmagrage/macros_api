...
class TinsleyMuscularWeightImplTest {
    ...
    @Autowired
    @InjectMocks
    private TinsleyMuscularWeightImpl tinsleyMuscularWeight;
    ...
    void calculate_shouldCalculateTheBasalSpentFromTinsleyMuscularWeightEquation(ProfileResponse profile, float expectedValue) {
        var result = tinsleyMuscularWeight.calculate(profile);
    ...
    void calculate_shouldThrowMissingValuesException_whenRequiredValuesAreMissing(ProfileResponse profile) {
        Assertions.assertThrows(MissingValuesInEquationException.class, () -> tinsleyMuscularWeight.calculate(profile));
    }
    ...
    static Stream<Arguments> provideProfile() {
        return Stream.of(
                ...
                Arguments.of(new ProfileResponse(null, 60.0f, 10.0f, LocalDateTime.parse("1995-06-30T00:00:00"), 165, 'F',
                        0.0f, null, false, true, PhaseEnum.MAINTENANCE, 15f, 500f), 1682.6f),
